...
int LFUCache::get(int key) {
   if (h_map.find(key) != h_map.end()) {
     ...
     list_map[h_map[key]->freq].push_back(p);
     it_map[p->key] = --list_map[h_map[key]->freq].end();
...
void LFUCache::put(int key, int value) {
  ...
  if (get(key) != -1) {
    ...
    size++;
    h_map[key] = p;
    list_map[1].push_back(p);
    it_map[key] = --list_map[1].end();
    // to be done
    if (size > capacity) {
      size--;
      Node *temp = list_map[minifreq].front();
      int var = temp->key;

      it_map.erase(var);
      h_map.erase(var);
      list_map[minifreq].pop_front();
    }
    minifreq = 1;
    return ;
}
