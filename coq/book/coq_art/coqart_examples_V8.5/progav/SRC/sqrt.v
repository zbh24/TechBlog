...
match p return {s:Z & (sqrt_type1 p s)} with
  1%positive => existS (sqrt_type1 1) 1 (exist (sqrt_type2 1 1) 0 th_sqrt1)
| 2%positive => existS (sqrt_type1 2) 1 (exist (sqrt_type2 2 1) 1 th_sqrt2)
| 3%positive => existS (sqrt_type1 3) 1 (exist (sqrt_type2 3 1) 2 th_sqrt3)
| xO (xO p') =>
  match sqrt_aux p' with
    (existS _ s' (exist _ r' h)) =>
    match Z_le_gt_dec  (4*s'+1) (4*r') with
      left h' =>
        (existS  (sqrt_type1 (xO (xO p'))) (2*s'+1)
           (exist  (sqrt_type2 (xO (xO p')) (2*s'+1)) ((4*r')-(4*s'+1)) 
                (th_sqrt_4_0_le p' s' r' h h')))
    | right h' =>
        (existS (sqrt_type1 (xO (xO p'))) (2*s')
