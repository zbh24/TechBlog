(* Exercise by Vincent Filou. Modified by Pierre Casteran *)
...
Use this result for building a projection from finite streams to
lists:
...
Proof.
 intros.
 induction H using Finite_rect_0.
 destruct x; auto.
...
Proof.
  ...
  induction H using Finite_rect.
  exists nil;auto.
  destruct IHFinite as [l0 H0].
 exists (x::l0);subst l;trivial. 
Defined.
...
Proof.
  intros x Hx;destruct (to_list_strong _ Hx).
  exact x0.
Defined.
...
Implicit Arguments Finite [A] .

Definition test:= (LCons 3 (LCons 2 (LCons 0 (@LNil nat)))).

Lemma test_finite: Finite  test.
  repeat constructor.
Defined.

Compute  (to_list nat test test_finite).
