...
apply
 (wf_inverse_image state Z (Zwf 0)
    ...
    (Zwf_well_founded 0)).
Qed.
(* If i is the Skip instruction, it always possible to decide whether
...
apply (result s i s2).
rewrite heq.
apply execScolon with s1; assumption.
...
Implicit Arguments Assign [Var aExp bExp].
Implicit Arguments WhileDo [Var aExp bExp].
Implicit Arguments Scolon [Var aExp bExp].
...
intros z tl s' v Heq; injection Heq; intros Heq1; rewrite <- Heq1.
trivial.
...
intros s'0 Heq; injection Heq; intros Heq1; rewrite <- Heq1.
trivial.
...
intros n i Heq s s' Hexec; generalize n Heq; clear Heq n.
elim Hexec.
auto.
simpl in |- *.
...
Theorem forLoops_fact3 :
 ...
 intros s; case s.
 intros s' Hevgt1 Hexec1.
 inversion Hexec1.
 inversion H2.
 simpl in H10; discriminate.
 ...
 generalize H8; clear H8; CaseEq (lookup 0 s).
...
Extraction "execute_loops.ml" execute_loops2 fact3 value.
