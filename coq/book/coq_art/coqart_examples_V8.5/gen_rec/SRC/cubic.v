...
refine (fix
        div8 (n : nat) : {q : nat & {r : nat | n = 8 * q + r /\ r < 8}} :=
           match n return {q : nat & {r : nat | n = 8 * q + r /\ r < 8}} with
              S (S (S (S (S (S (S (S x))))))) =>
                match div8 x with existS _ q' (exist _ r (conj Heq Hlt)) => _ end
             | _ => _
           end).
...
refine (fun n cubic =>
           match div8_spec n with
             existS _ (S q) (exist _ r8 (conj Heq Hltr8)) =>
               match cubic (S q) _ with
                 existS _ c' (exist _ r (conj Heqc Hltr)) =>
                   match le_lt_dec ((12 * (c' * c') + 6 * c') + 1) (8 * r + r8)
                    with left Hle => _ | right Hlt => _ end
               end
            | existS _ 0 (exist _ 0 (conj Heq _)) => _
            | existS _ 0 (exist _ (S n') (conj Heq Hlt)) => _
           end).
