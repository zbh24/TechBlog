...
(* We use the refine tactic and we hope this makes the algorithm
 ...
 refine
   (fun l0 rec =>
      match rec l0 _ with
      | inleft (existS _ (close::l1) (exist _ t1 (conj Hp Hl))) =>
        match rec l1 _ with
        | inleft (existS _ l2 (exist _ t2 (conj Hp2 Hl2))) =>
          inleft _
              (existS  (fun l' => {t:bin | parse_rel (open::l0) l' t /\
                                          length l' <= S (length l0)})
                   l2 (exist _ (N t1 t2) _))
        | inright Hnp => inright _ (fun l'' t' Hp' => _)
        end
      | inleft (existS _ (open::l1) (exist _ t1 (conj Hp Hl))) => 
         inright _ (fun l'' t' Hp' => _)
      | inleft (existS _ nil (exist _ t1 (conj Hp Hl))) =>
         inright _ (fun l'' t' Hp' => _)
      | inright Hnp => inright _ (fun l'' t' Hp' => _)
      end);(auto ||
            (try (inversion Hp';
