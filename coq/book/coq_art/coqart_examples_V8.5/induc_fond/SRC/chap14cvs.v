...
Definition nat_case
  (F:nat->Set)(exp1:F 0)(exp2:forall p:nat, F (S p))(n:nat) :=
...
Module Type nat_rec_type.
Parameter
nat_rec :
  forall f:nat->Set, f 0 ->(forall n:nat, f n -> f (S n))-> forall n:nat, f n.
End nat_rec_type.

Module nat_rec_module : nat_rec_type.
...
Module Type Z_btree_rec_type.
Parameter 
Z_btree_rec :
  forall f:Z_btree->Set,
   f Z_leaf ->
   (forall (n:Z)(t1:Z_btree), f t1 ->
      forall t2:Z_btree, f t2 -> f (Z_bnode n t1 t2))->
   forall t:Z_btree, f t.
End Z_btree_rec_type.

Module Z_btree_rec_module : Z_btree_rec_type.
...
(** ...*)
(**  IHm : forall n:nat, n <= m -> rich_minus m n*)
(**  n : nat*)
(**  Hle : 0 <= S m*)
(**  ============================*)
(**   S m + 0 = S m*)
...
Check (forall (A:Set)(u v:list A), lfactor u v -> {w : list A | v = app u w}).
...
End update_def.
Check (fun eq_rec_eq
     :forall (U:Set)(p:U)(Q:U->Set)(x:Q p)(h:p = p),
...
Proof.
 ...
 auto with zarith.
 intros t1 Hrec1 tl Hrec2 Hocc; lazy beta iota delta -[Zplus Zle];
