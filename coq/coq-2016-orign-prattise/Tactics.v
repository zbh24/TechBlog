...
(** This chapter introduces several more proof strategies and
    ...
    - how to use auxiliary lemmas inboth "forward-style" and
...
Proof.
  ...
  destruct m.
    ...
    simpl in H1.
    symmetry in H1.
    rewrite plus_comm with (n:=m) (m:=(S m)) in H1.
    simpl in H1.
    inversion H1.
    symmetry in H2.
    apply IHn' in H2.
...
Lemma app_length_equal1:forall (X:Type) (l:list X) (n:X),length (l ++ [n]) = S (length l).
Proof.
induction l;intros.
simpl.
reflexivity.
...
- (*  l = cons n' l' *)
  intros.
  destruct l2.
  + rewrite app_right_poly.
    rewrite app_length_equal1 in H.
    rewrite H.
    reflexivity.
  + simpl in H.
    simpl.
...
Lemma combine_split_refl:forall X  Y (l:list (X * Y) ),combine (fst (split l)) (snd (split l)) = l.
Proof.
induction l.
intros.
simpl.
...
inversion H.
simpl.
assert (HP: p = (fst p ,snd p)).
   destruct p.
   simpl.
   reflexivity.
rewrite <- HP.
rewrite combine_split_refl.
reflexivity.
Qed.
...
intros f b.
destruct b.
destruct (f true) eqn:HX.
rewrite HX.
apply HX.
destruct (f false) eqn:HX1.
rewrite HX.
reflexivity.
apply HX1.
destruct (f false) eqn:HX.
destruct (f true) eqn:HX1.
apply HX1.
apply HX.
rewrite HX.
apply HX.
...
Definition split_combine_statement : Prop := forall X Y (l1:list X) (l2:list Y) (l:list (X * Y)),
...
Fixpoint forallb {X:Type} (f:X->bool) (l:list X) :=
  match l with
      | nil => true
      | cons n l' => if f n then forallb f l' else false
  end.

Fixpoint existsb {X:Type} (f:X->bool) (l:list X) :=
  match l with
      | nil => false
      | cons n l' => if f n then true else existsb f l'
  end.

Definition existsb' {X:Type} (f:X->bool) (l:list X) :bool := negb (forallb (fun (x:X) => negb (f x)) l ).

Example test_exist1: existsb (andb true) [true;true;false] = true.
Proof.
unfold existsb'.
compute.
reflexivity.
...
Lemma existb_existsb':forall X f (l:list X),existsb f l = existsb' f l.
Proof.
induction l as [| n l'].
-
simpl.
