...
(** In the [Logic] chapter we looked at several ways of writing
    ...
    we can establish its evenness from inthe following rules:
...
Theorem evenb_minus20: forall n,
  evenb n = true -> evenb (pred (pred n)) = true.
Proof.
  intros n.
  destruct n.
...
(*
It can't use the destruct and induction
*)
...
intros.
SearchAbout ev.
apply ev_ev__ev with (n:=n) (m:=m) in H.
...
Check (ev_0).
Check (ev_SS 0 ev_0).
Check (ne_1 1 (ev_SS 0 ev_0)).
Check (ne_2 0 (ev_SS 0 ev_0)).

(** **** Exercise: 2 stars, recommended (total_relation)  *)
(** Define an inductive binary relation [total_relation] that holds
    between every pair of natural numbers. *)
Inductive binary_relation:nat->nat->Prop :=
  b_re:forall (n m:nat), binary_relation n m.
...
Inductive empty_relation:nat->nat->Prop :=
  b_empty:forall n,ev n -> ~ev n -> empty_relation n n.
...
Lemma leb_snm:forall n m, n <= m -> n <= S m.
intros n m H.
inversion H.
apply le_S.
apply le_n.
...
Lemma le_Sn_O:forall m  ,(S m) <= 0->leb (S m) 0 = true.
Proof.
induction m.
intros.
inversion H.
...
Inductive subseq:list nat -> list nat -> Prop :=
  | sub_same: forall (l:list nat),subseq l l

  | sub_forward: forall (l1 l2 l3:list nat),
                   subseq l1 l2 -> subseq l1 (app l3 l2)

  | sub_backward: forall (l1 l2 l3:list nat),
                   subseq l1 l2 -> subseq l1 (app l2 l3).
...
Lemma  subseq_app:forall l1 l2 l3,subseq l1 l2 -> subseq l1 (app l2 l3).
Proof.
intros.
apply sub_backward.
apply H.
...
induction T23 as [l0 | l1 l2 l3 | l1 l2 l3 ];intros.
apply T12.
apply sub_forward.
apply IHT23.
apply T12.
...
Inductive RQ : nat -> list nat -> Prop :=
      | c1 : RQ 0 []
      | c2 : forall n l, RQ n l -> RQ (S n) (n :: l)
      | c3 : forall n l, RQ (S n) l -> RQ n l.

Example R_example1:RQ 2 [1;0].
Proof.
apply c2.
apply c2.
apply c1.
...
(** [] *)
(* Example R_example2:RQ 1 [1;2;1;0]. *)
(* Proof. *)
(* apply c3. *)
(* apply c3. *)
(* apply c3. *)
