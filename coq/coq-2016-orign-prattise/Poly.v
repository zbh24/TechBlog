...
Lemma app_right_poly: forall X (l :list X),
                         l ++ [] = l.
Proof.
...
(* Wrong *)
(* Lemma app_comm:forall X (l1 l2 :list X), l1 ++ l2 = l2 ++ l1. *)
(* Proof. *)
...
Lemma map_rev_lemma:forall (X Y:Type) (l:list X) (n:X) (f:X-> Y), 
...
Proof.
  (* FILL IN HERE *) 
induction l as [| n l' IHl];intros;simpl.
reflexivity.
simpl.
...
(* FILL IN HERE *)
fold (fun (a:X) (l2:list Y) => cons (f a) l2) l [].
...
Definition test_f {X:Type}(x y:X):X->X := fun x:X=> x.
Check @test_f.
...
Definition mult (n m : nat) : nat :=
  (* FILL IN HERE *) fun (X:Type) (f:X->X)(x:X) => n X (plus m zero X f) x.   
