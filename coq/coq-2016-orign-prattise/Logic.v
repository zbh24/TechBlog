...
Lemma in_app3:forall X (l l':list X) (a:X) (x:X),
In x (l ++ l') -> In x (l ++ a::l').
Proof.
induction l.
-
...
Lemma In_comm:forall X (l l':list X) (a:X) ,In a (l ++ l') -> In a (l'++l ).
Proof.
...
Lemma in_app:forall X (l l':list X) (a:X) ,In a l -> In a (l++l').   
Proof.
induction l;intros.
inversion H.
simpl in H.
...
simpl.
reflexivity.
destruct IHn as [k1 HK].
destruct (evenb (S n) ) eqn:HX.
rewrite evenb_S in HX.
...
rewrite <- ha in HX.
rewrite evenb_S in HX.
destruct (evenb n);simpl in HX;inversion HX.
Qed.
...
Theorem equal_double_negation_elimination:
